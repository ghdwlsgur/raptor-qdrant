...
class Ollama(BaseChatbotModel):
    def __init__(
        ...
        logger.info(f"ollama client created for model {self.model} at {self.host}")
    ...
    def answer(self, context: str, question: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"ollama api call failed: {e.code} {detail}")
        except (urllib.error.URLError, TimeoutError) as e:
            logger.error(f"error calling ollama: {e}")
            raise RuntimeError(f"ollama api call failed: {e}")
        except (KeyError, json.JSONDecodeError) as e:
            logger.error(f"unexpected ollama response shape: {e}")
            raise RuntimeError(f"ollama returned an unreadable response: {e}")
