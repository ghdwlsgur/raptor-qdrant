...
def reduce_embedding_dimensions(
    ...
    if len(embeddings) <= 2:
        logger.warning(f"Too few embeddings ({len(embeddings)}) for UMAP reduction, returning original")
    ...
    if n_neighbors is None:
        n_neighbors = max(2, min(int((len(embeddings) - 1) ** 0.5), len(embeddings) - 1))
